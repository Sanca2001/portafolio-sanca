...
export default function ProjectsGrid({ items = proyectos }: ProjectsGridProps) {
  return (
    <div className="w-full pb-24" id="projects-grid">
        ...
        {items.map((proyecto, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`w-full aspect-[4/3] ${bgGradient} rounded-[2rem] p-6 sm:p-8 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:shadow-lg group-hover:-translate-y-1 relative border border-white/60`}
              >
                <div className="w-full h-full relative rounded-xl overflow-hidden shadow-md group-hover:shadow-xl transition-shadow duration-300 bg-white/90 flex items-center justify-center">
                  <img
                    src={proyecto.imagen || proyecto.mockup}
                    alt={proyecto.titulo}
                    className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.03]"
                    onError={(e) => {
                      // Fallback preview styling if image broken
              ...
              <div className="mt-3.5 px-1 flex items-baseline justify-between gap-4">
                <h3 className="text-gray-900 font-medium text-sm sm:text-base group-hover:text-black transition-colors line-clamp-1">
                  {proyecto.titulo}
                </h3>
                <span className="text-gray-400 text-xs sm:text-sm whitespace-nowrap font-normal">
                  {primaryTag}
                </span>
              </div>
            </Link>
